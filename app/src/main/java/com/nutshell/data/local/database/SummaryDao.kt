...
interface SummaryDao {
    ...
    @Query(
        "SELECT * FROM summaries WHERE originalText LIKE '%' || :query || '%' OR shortSummary LIKE '%' || :query || '%' OR mediumSummary LIKE '%' || :query || '%' OR detailedSummary LIKE '%' || :query || '%' ORDER BY createdAt DESC",
    )
    fun searchSummaries(query: String): Flow<List<SummaryEntity>>

    @Query(
        "SELECT * FROM summaries WHERE isSaved = 1 AND (originalText LIKE '%' || :query || '%' OR shortSummary LIKE '%' || :query || '%' OR mediumSummary LIKE '%' || :query || '%' OR detailedSummary LIKE '%' || :query || '%') ORDER BY createdAt DESC",
    )
