...
class SummaryRemoteDataSourceImpl @Inject constructor(
    ...
    override suspend fun summarizeText(request: SummarizeRequest): ApiResult<SummarizeResponse> {
        ...
        for (attempt in 1..maxRetries) {
            try {
                println(
                    "SummaryRemoteDataSource: Making API request (attempt $attempt/$maxRetries) with text length: ${request.text.length}",
                )
                ...
                lastException = e
                println(
                    "SummaryRemoteDataSource: API request failed (attempt $attempt/$maxRetries): ${e.javaClass.simpleName}: ${e.message}",
                )
                e.printStackTrace()
